# 🌐 Personal Portfolio Website

A modern and responsive **Personal Portfolio Website** built using **HTML, CSS, and JavaScript**.  
This portfolio showcases my skills, projects, and experience with interactive UI, animations, and theme customization.

---

## 🚀 Live Demo

🔗 https://your-portfolio-link-here

---

## 📂 GitHub Repository

🔗 https://github.com/aayushgupta-4341/your-portfolio-repo

---

## ✨ Features

* 🎯 Clean and modern UI design  
* ⌨️ Typing animation in hero section  
* 🧊 Glassmorphism UI effect  
* 🎨 Smooth hover and 3D card animations  
* 🌗 Dark / Light mode toggle (with icon change)  
* 📱 Fully responsive design  
* 📑 Sections: Home, About, Projects, Experience, Contact  
* 🔗 Social media icons with redirect links  
* ⬇️ Download resume button  

---

## 🛠️ Tech Stack

* HTML  
* CSS  
* JavaScript  

---

## 📸 Screenshot

(Add your portfolio screenshot here)

---

## 📚 What I Learned

* Designing real-world portfolio layouts  
* Implementing typing animation using JavaScript  
* Creating theme toggle (dark/light mode)  
* Building reusable UI sections  
* Adding smooth animations and transitions  
* Improving responsiveness across devices  

---

## 💼 About Me

* 👨‍💻 MERN Stack Developer  
* 🎓 BSc IT (Final Year) – B.N.N College  
* 📊 Current Pointer: ~9  
* 🏫 10th: B.V.N Hindi High School (86.20%)  
* 🏫 12th: B.N.N College (55%)  

---

## 💡 Future Improvements

* Add project filtering system  
* Integrate GitHub API for dynamic projects  
* Add advanced animations (GSAP)  
* Add contact form with backend  
* Improve UI/UX with more 3D effects  

---

## 📅 Challenge

This project is part of my **30 Days Web Development Challenge**  
📌 Day 28 – Portfolio Website  

---

## 🙌 Connect With Me

* 💼 LinkedIn: https://www.linkedin.com/in/aayush-gupta4341  
* 💻 GitHub: https://github.com/aayushgupta-4341  
* 📧 Email: ayushgupta69904@gmail.com  

---

⭐ If you like this project, don't forget to star the repository!
